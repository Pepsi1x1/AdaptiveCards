...
namespace AdaptiveCards.Rendering.Wpf
{
    public class AdaptiveXamarinDialectXamlMarkdownRenderer : MarkdownRenderer
    {
        ...
        public override string Postprocess(string text)
        {
            ...
            string rets = string.Join("\r", processedLines);

            rets = rets.Replace(NEWLINE_MARKER, NEWLINE_SPAN_TEXT);

            rets = $"<FormattedString>{rets}</FormattedString>";

            return base.Postprocess(rets);
        }
    }
}

...
namespace AdaptiveCards.Rendering.Wpf
{
    public static class AdaptiveContainerRenderer
    {
        ...
        /// <param name="uiContainer">Container of rendered elements</param>
        /// <returns>Spacing grid</returns>
        public static Grid AddSpacing(AdaptiveRenderContext context, AdaptiveSpacing spacing, Grid uiContainer)
        {
            ...
            if (element is AdaptiveInput)
            {
                ...
                if (inputElement is AdaptiveChoiceSetInput)
                {
                    if (renderedElement is Grid)
                    {
                        ...
                        FrameworkElement choiceSet = inputContainer.Children[0];
...
#elif XAMARIN
                    Frame visualCue = new Frame();
                    visualCue.HasShadow = false;
                    visualCue.Content = renderedElement;
                    visualCue.BorderColor = context.GetColor(context.GetForegroundColors(AdaptiveTextColor.Attention).Default);
#endif
        ...
        public static FrameworkElement GetVisualElementForAccessibility(AdaptiveRenderContext context, AdaptiveInput input)
        {
